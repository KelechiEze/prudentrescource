...
// 'use client';
// import React, { useState, useMemo, useRef, useEffect } from 'react';
// import { Search, ChevronDown, ChevronLeft, ChevronRight, Check, X, ExternalLink, Trash2, UploadCloud, Eye, Download, ShieldCheck, Loader2 } from 'lucide-react';
...
//       if (credentialFilter !== 'All Credentials') queryParams.append('credentials', credentialFilter);
//       // Note: availability filter might need backend support
...
//   const fetchProfessional = async (id: number): Promise<Professional | null> => {
//     try {
//       const queryParams = new URLSearchParams({
//         action: 'get',
//         id: id.toString(),
...
//   // Update professional
//   const updateProfessional = async (professional: Professional): Promise<boolean> => {
//     try {
//       const queryParams = new URLSearchParams({
//         action: 'update',
...
//   useEffect(() => {
//     fetchProfessionals();
//   }, [currentPage, searchQuery, credentialFilter, sortFilter]);
...
//     // Apply availability filter client-side if not supported by backend
//     if (availabilityFilter !== "All Availability") {
//       result = result.filter(prof => prof.availability === availabilityFilter);
//     }
    
//     return result;
//   }, [professionals, availabilityFilter]);
...
//   const startIndex = (currentPage - 1) * itemsPerPage;
//   const currentItems = filteredProfessionals.slice(0, itemsPerPage); // Already paginated by API
...
//   const handleUpdateProfessional = async (updated: Professional) => {
//     const success = await updateProfessional(updated);
//     if (success) {
//       // Update local state
//       setProfessionals(prev => prev.map(p => p.id === updated.id ? updated : p));
...
//                   <tr>
//                     <td colSpan={5} className="px-8 py-20 text-center text-gray-500">
//                       {error ? 'Failed to load professionals. Please try again.' : 'No professionals found matching your search and filters.'}
//                     </td>
//                   </tr>
//                 )}
//               </tbody>
...
//         <div className="mt-8 flex flex-col sm:flex-row items-center justify-between gap-6">
//           <p className="text-[14px] text-gray-500 font-medium">
//             Showing <span className="text-[#1B2C42] font-bold">{filteredProfessionals.length > 0 ? startIndex + 1 : 0} - {Math.min(startIndex + itemsPerPage, totalItems)}</span> of {totalItems}
//           </p>
...
//           onClose={() => setSelectedProfessionalId(null)} 
//           onUpdate={handleUpdateProfessional}
//           fetchProfessional={fetchProfessional}
//         />
//       )}
//     </div>
//   );
...
//   onClose: () => void;
//   onUpdate: (prof: Professional) => void;
//   fetchProfessional: (id: number) => Promise<Professional | null>;
// }

// const ProfessionalModal: React.FC<ProfessionalModalProps> = ({ professionalId, onClose, onUpdate, fetchProfessional }) => {
...
//     setFormData(prev => prev ? { ...prev, [field]: undefined } : null);
//   };
...
//     const file = e.target.files?.[0];
//     if (file) {
//       setFormData(prev => prev ? { ...prev, [field]: file.name } : null);
//     }
//   };
...
//             <p className="text-gray-600 mb-4">Failed to load professional details.</p>
//             <button 
//               onClick={onClose}
...
//               onClick={handleSaveChanges}
//               disabled={saving}
//               className="w-full h-[58px] bg-[#1B2C42] hover:bg-[#152336] text-white rounded-full font-bold text-[16px] transition-all active:scale-[0.98] shadow-lg flex items-center justify-center gap-3 disabled:opacity-50 disabled:cursor-not-allowed"
//             >
//               {saving ? (
//                 <>
...
const AdminDashboardPage: React.FC<{ onLogout: () => void; onNavigate: (page: string) => void; currentPage: string; }> = ({ onLogout, onNavigate, currentPage: activePageId }) => {
  ...
  const currentItems = filteredProfessionals.slice(0, itemsPerPage);
  ...
  return (
    <div className="min-h-screen bg-white font-sans text-[#1B2C42]">
      <main className="max-w-[1440px] mx-auto px-8 md:px-12 py-10">
        ...
        <div className="mt-8 flex flex-col sm:flex-row items-center justify-between gap-6">
          <p className="text-[14px] text-gray-500 font-medium">
            Showing <span className="text-[#1B2C42] font-bold">{filteredProfessionals.length > 0 ? (currentPage - 1) * itemsPerPage + 1 : 0} - {Math.min(currentPage * itemsPerPage, totalItems)}</span> of {totalItems}
          </p>
          {filteredProfessionals.length > 0 && totalPages > 1 && (
            <div className="flex items-center gap-2">
              <button onClick={() => handlePageChange(currentPage - 1)} disabled={currentPage === 1} className="w-10 h-10 flex items-center justify-center hover:bg-gray-100 rounded-full transition-colors disabled:opacity-30">
...
interface ProfessionalModalProps {
  professionalId: number;
  onClose: () => void;
  onUpdate: (prof: Professional) => Promise<boolean>;
  fetchProfessional: (id: number) => Promise<Professional | null>;
}
...
const ProfessionalModal: React.FC<ProfessionalModalProps> = ({ professionalId, onClose, onUpdate, fetchProfessional }) => {
  ...
  const [saving, setSaving] = useState(false);
  const [formData, setFormData] = useState<Professional | null>(null);
  const [originalData, setOriginalData] = useState<Professional | null>(null);
  const [isStatusPickerOpen, setIsStatusPickerOpen] = useState(false);
  const [viewingPdf, setViewingPdf] = useState<{ name: string, type: string } | null>(null);
  ...
  useEffect(() => {
    if (formData && originalData) {
      // Check if any field has changed (except availability which is handled separately)
      const fieldsToCheck = ['firstName', 'lastName', 'email', 'city', 'state', 'phone', 'role', 'credentials'];
      const changed = fieldsToCheck.some(key => {
        return formData[key as keyof Professional] !== originalData[key as keyof Professional];
      });
      
      // Also check availability separately
      const availabilityChanged = formData.availability !== originalData.availability;
      
      setHasChanges(changed || availabilityChanged);
    }
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      ...
      if (success) {
        await loadProfessional(); // Refresh to get updated data
        setIsEditMode(false);
        setHasChanges(false);
      } else {
        alert('Failed to save changes. Please try again.');
  ...
  const handleDownload = async (type: 'resume' | 'cert', fileName?: string) => {
    try {
      const action = type === 'resume' ? 'download-resume' : 'download-cert';
      const response = await fetch(`/api/resume/submit?action=${action}&id=${professionalId}`);
      ...
      const link = document.createElement('a');
      link.href = url;
      link.setAttribute('download', fileName || `${type}_${professionalId}.pdf`);
      document.body.appendChild(link);
      link.click();
  ...
  const handleOpenFile = (type: 'resume' | 'cert', fileName?: string) => {
    const action = type === 'resume' ? 'download-resume' : 'download-cert';
    const url = `/api/resume/submit?action=${action}&id=${professionalId}`;
      ...
      <div className="flex items-center justify-between p-4 bg-gray-50/50 rounded-[16px] border border-gray-100 group transition-all hover:bg-white hover:shadow-sm">
        ...
        <div className="flex items-center gap-2">
          <button 
            ...
            className="p-2.5 text-gray-400 hover:text-[#68cfa3] hover:bg-teal-50 rounded-full transition-all active:scale-90"
            title="Download File"
          >
            <Download size={18} />
          </button>
        ...
        <div className="px-12 py-12 h-full overflow-y-auto max-h-[95vh] no-scrollbar">
          <div className="flex items-start justify-between mb-2">
            ...
            {!isEditMode ? (
              ...
              <button 
                ...
                className="flex items-center gap-2 text-gray-500 font-bold text-[15px] hover:text-red-500 hover:underline transition-all pt-2 pr-10"
              >
                Cancel <X size={16} />
              </button>
            )}
            ...
            <div className="grid grid-cols-2 gap-6">
              <div className="space-y-2.5">
                ...
                <input 
                  ...
                  onChange={handleInputChange} 
                  disabled={!isEditMode}
                  className={`w-full h-[54px] px-5 border rounded-[12px] bg-white text-[15px] focus:outline-none transition-colors shadow-sm ${
                    isEditMode 
                      ? 'border-[#68cfa3] focus:border-[#68cfa3]' 
          ...
          <div className="mt-12 space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <FileItem 
                ...
                field="resume" 
                inputRef={resumeRef}
                onViewClick={() => handleOpenFile('resume', formData.resume)}
                onDownloadClick={() => handleDownload('resume', formData.resume)}
              />
              <FileItem 
                label="Certification" 
                fileName={formData.certificate} 
                field="certificate" 
                inputRef={certRef}
                onViewClick={() => handleOpenFile('cert', formData.certificate)}
                onDownloadClick={() => handleDownload('cert', formData.certificate)}
              />
            </div>
          </div>
          ...
          <div className="mt-14 pt-2 flex items-center justify-between">
            ...
            <button 
              onClick={toggleAvailability}
              disabled={!isEditMode}
              className={`w-[68px] h-[34px] rounded-full p-1.5 transition-all duration-300 flex items-center ${
                formData.availability === 'Open' ? 'bg-[#68cfa3]' : 'bg-gray-200'
              } ${!isEditMode ? 'opacity-50 cursor-not-allowed' : ''}`}
            >
              <div className={`w-[22px] h-[22px] bg-white rounded-full shadow-md transition-all duration-300 transform ${
                formData.availability === 'Open' ? 'translate-x-[34px]' : 'translate-x-0'
              }`}></div>
            </button>
          </div>
          ...
          {isEditMode && hasChanges && (
            <div className="mt-12 pb-4">
              <button 
                onClick={handleSaveChanges}
                disabled={saving}
                className="w-full h-[58px] bg-[#68cfa3] hover:bg-[#5abf94] text-white rounded-full font-bold text-[16px] transition-all active:scale-[0.98] shadow-lg flex items-center justify-center gap-3 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {saving ? (
                  <>
                    <Loader2 className="w-5 h-5 animate-spin" />
            ...
            <div className="relative bg-white w-full max-w-[440px] rounded-[24px] shadow-2xl border border-gray-100 p-8 animate-in fade-in zoom-in-95 duration-300">
              ...
              <div className="flex flex-col items-center text-center">
                ...
                <div className="w-full space-y-3 mb-8">
                  <div className="flex items-center justify-between py-3 border-b border-gray-50">
                    <span className="text-[13px] text-gray-400 font-bold uppercase tracking-wider">File Status</span>
                    <span className="text-[13px] text-green-600 font-bold flex items-center gap-1.5">
                ...
                <div className="grid grid-cols-2 gap-3 w-full">
                  <button 
                    onClick={() => {
                      const type = viewingPdf.type === 'Resume/CV' ? 'resume' : 'cert';
                      handleDownload(type, viewingPdf.name);
                    }}
                    className="flex items-center justify-center gap-2 h-12 rounded-full border border-gray-200 text-[#1B2C42] font-bold text-[14px] hover:bg-gray-50 transition-colors active:scale-95"
                  >
                    <Download size={16} /> Download
                  ...
                  <button 
                    onClick={() => {
                      const type = viewingPdf.type === 'Resume/CV' ? 'resume' : 'cert';
                      handleOpenFile(type, viewingPdf.name);
                    }}
                    className="flex items-center justify-center gap-2 h-12 rounded-full bg-[#68cfa3] text-[#1B2C42] font-bold text-[14px] hover:bg-[#5abf94] transition-colors active:scale-95"
                  >
                    <Eye size={16} /> Open File
