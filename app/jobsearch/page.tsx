      ...
      <div className="bg-white min-h-screen pt-32 md:pt-40">
        <div className="max-w-7xl mx-auto px-4 md:px-6 pb-20 md:pb-32">
          <button 
            ...
            className="flex items-center gap-2 px-4 py-2 bg-[#E3E8DE] hover:bg-[#D8DDD3] text-[#1B2C42] rounded-full text-[12px] md:text-[13px] font-bold transition-all mb-8 md:mb-10 w-fit active:scale-95"
          >
          ...
          <div className="flex flex-wrap gap-2 md:gap-3 mb-10 md:mb-12">
            {[selectedJob.category, selectedJob.datePosted, selectedJob.location, selectedJob.type].map((tag, i) => (
              <div key={i} className="px-4 md:px-6 py-2 md:py-3 bg-[#F4F6F8] rounded-full text-[#1B2C42] text-[12px] md:text-sm font-bold shadow-sm whitespace-nowrap">
                {tag}
              </div>
            ))}
          </div>
          ...
          <div className="max-w-7xl space-y-10 md:space-y-12 text-[#1B2C42]/90 leading-relaxed text-base md:text-[18px]">
            ...
            <div className="pt-8 md:pt-12">
              <button className="w-full sm:w-auto bg-[#68cfa3] hover:bg-[#5abf94] text-white px-12 py-4 rounded-full text-[14px] md:text-[15px] font-bold flex items-center justify-center gap-2 transition-all shadow-xl shadow-teal-500/10 active:scale-95">
    ...
    <div className="bg-white min-h-screen flex flex-col">
      <div className="max-w-7xl mx-auto px-4 md:px-6 pt-32 md:pt-40 pb-20 md:pb-32 w-full">
        ...
        <div className="max-w-3xl mx-auto mb-10 md:mb-16 flex flex-col sm:flex-row gap-3">
          <div className="flex-1 bg-[#E3E8DE]/60 rounded-full h-12 md:h-14 px-6 md:px-8 flex items-center gap-3 border border-transparent focus-within:border-teal-400/50 transition-all shadow-sm">
            ...
            <input 
              ...
              placeholder="Search discipline or location" 
            ...
            />
            {searchQuery && (
              <button onClick={() => setSearchQuery("")} className="p-1.5 hover:bg-gray-200 rounded-full transition-colors">
                <X size={14} className="text-gray-400" />
              </button>
            )}
          </div>
          <button className="h-12 md:h-14 px-8 md:px-10 bg-[#1B2C42] hover:bg-[#152336] text-white text-sm md:text-base font-bold rounded-full flex items-center justify-center gap-2 transition-transform active:scale-95 shadow-lg whitespace-nowrap">
        ...
        <div className="relative flex flex-wrap items-center gap-x-4 md:gap-x-10 gap-y-5 mb-10 md:mb-14 border-b border-gray-100 pb-5 z-30" ref={dropdownRef}>
            ...
            {[
              ...
              <div key={filter.type} className="relative">
                <div 
                  ...
                  className={`flex items-center gap-2 group cursor-pointer border-b-2 pb-1 transition-colors whitespace-nowrap ${filter.label.includes('All') ? 'border-gray-400' : 'border-teal-500'}`}
                >
                    ...
                    <div className="max-h-[300px] overflow-y-auto no-scrollbar">
                      {filter.options.map(opt => (
                        <div 
                          ...
                          className={`px-4 py-3 text-sm hover:bg-gray-50 transition-colors ${filter.label === opt ? 'bg-teal-50 text-teal-600 font-bold' : 'text-gray-700'}`}
                        >
                          {opt}
                        </div>
                      ))}
          ...
          <button 
            onClick={clearAll} 
            className="text-[#1B2C42] font-bold text-[12px] md:text-sm hover:underline flex items-center gap-1.5 opacity-70 hover:opacity-100 transition-opacity whitespace-nowrap"
          >
            <X size={14} /> Clear All
          </button>
        </div>
        ...
        {filteredJobs.length > 0 ? (
              ...
              >
                <div className="mb-6">
                  <div className="flex justify-between items-start mb-4">
                    <p className="text-[10px] md:text-[11px] text-gray-500 font-bold uppercase tracking-wider line-clamp-1 flex-1 mr-2">
                      {job.category} • {job.schedule} • {job.duration}
                    </p>
                    <span className="bg-white/50 text-[9px] md:text-[10px] px-2 py-0.5 rounded font-bold text-[#1B2C42] uppercase tracking-tighter whitespace-nowrap">
                      {job.type}
                    </span>
                  </div>
                  <h3 className="font-serif text-2xl md:text-[28px] text-[#1B2C42] leading-tight mb-6 min-h-[60px] md:min-h-[70px] group-hover:text-teal-600 transition-colors">
                    {job.title}
                  </h3>
                  <div className="space-y-1">
                    <p className="text-gray-600 text-sm font-medium">{job.location}</p>
                    <p className="text-gray-500 text-[11px] md:text-[12px]">Date Posted: {job.datePosted}</p>
                  </div>
                </div>

                <div className="mt-auto pt-6 border-t border-gray-300/30">
                  <p className="text-[#1B2C42] text-lg md:text-xl font-bold mb-5 md:mb-6">
                    Starting at <span className="font-black">{job.startingPrice}</span>
                  </p>
                  <button className="w-full bg-teal-400 hover:bg-teal-500 text-[#1B2C42] py-3.5 md:py-4 rounded-full text-sm font-bold transition-all shadow-sm active:scale-95">
                    {job.buttonLabel}
                  </button>
                </div>
              </div>
          ...
          <div className="py-16 md:py-24 text-center flex flex-col items-center justify-center bg-gray-50 rounded-xl border-2 border-dashed border-gray-200 px-6">
            ...
            <button onClick={clearAll} className="bg-[#1B2C42] text-white px-8 py-3 rounded-full font-bold hover:bg-[#152336] transition-all active:scale-95">
              Reset Filters
            </button>
          </div>
        )}
