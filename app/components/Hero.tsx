    ...
    <section className="relative w-full min-h-screen flex flex-col justify-center overflow-hidden bg-black">
      ...
      <div className="relative z-10 w-full px-6 md:px-10 lg:px-20 pt-40 pb-20">
        <div className="mb-[15rem]">
          <h1 className="font-serif text-3xl md:text-4xl lg:text-[85px] leading-[110%] mb-10 mt-20 text-white font-normal tracking-tight">
        ...
        <div className="flex flex-col lg:flex-row items-start lg:items-end justify-between gap-12 w-full">
          ...
          <div className="flex flex-col sm:flex-row items-center gap-4 w-full max-w-2xl">
            <div className="flex-1 w-full bg-transparent rounded-full h-14 px-8 flex items-center gap-4 border border-white/20 transition-all focus-within:bg-white/20 focus-within:border-teal-400/50">
              <Search className="text-white/60 w-3 h-3" />
              <input 
                type="text" 
                placeholder="Search your discipline" 
                className="bg-transparent border-none outline-none text-white placeholder-white font-bold w-full h-full text-lg font-light"
              />
            </div>
            <button 
              onClick={onSearch}
              className="w-full sm:w-auto h-14 px-12 bg-teal-400 hover:bg-teal-500 text-gray-900 text-[14px] font-bold rounded-full flex items-center justify-center gap-2 transition-all uppercase tracking-widest shadow-[0_0_30px_rgba(104,207,163,0.3)] active:scale-95 whitespace-nowrap"
            >
